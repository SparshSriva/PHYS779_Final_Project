            ...
            if operation == "s-split":
                ...
                for i in unused_outputs:
                    ...
                    if q in used_qubits:
                        continue
                    if i not in graph_r.vertex_set():
                        n_output = graph_r.add_vertex(VertexType.BOUNDARY, qubit=q, row=graph_r.depth())
                        temp_outputs.append(n_output)
                ...
                compose_by_qubit(self.total_graph, graph_r)

            elif operation == "r-split":
                temp_inputs = []
                temp_outputs = []

                if len(qubits) != 3:
                    raise ValueError("r-split operation requires 3 qubit indices: input, output1, output2")
                q_in, q_out1, q_out2 = qubits
                ...
                upper, lower, inp_v = create_rough_split(graph_r, self.total_graph, q_in, q_out1, q_out2, measurement_label)
...
def create_rough_split(graph: Graph, full_graph: Graph, inp: int, out1: int, out2: int, meas_outcome):
    ...
    # Add Z-spider 
    graph.add_vertex_indexed(vetices_full_graph)
    graph.set_type(vetices_full_graph, VertexType.X)
    graph.set_phase(vetices_full_graph, 0)
    graph.set_row(vetices_full_graph, row_base + 1)
    graph.set_qubit(vetices_full_graph, inp)
    ...
    # Add X correction spider 
    graph.add_vertex_indexed(vetices_full_graph+1)
    graph.set_type(vetices_full_graph+1, VertexType.Z)
    graph.set_phase(vetices_full_graph+1, meas_outcome)
    graph.set_row(vetices_full_graph+1, row_base + 2)
    graph.set_qubit(vetices_full_graph+1, out1)
...
def create_smooth_merge(graph: Graph, full_graph: Graph, in1: int, in2: int, out: int, meas_outcome, pauli_frame: PauliFrame=None):
    ...
    graph.add_vertex_indexed(vetices_full_graph)
    graph.set_type(vetices_full_graph, VertexType.X)
    graph.set_phase(vetices_full_graph, meas_outcome)
    graph.set_row(vetices_full_graph, full_graph.row(q_in_1))
    graph.set_qubit(vetices_full_graph, in1)
    
    ## Merge merge = graph.add_vertex(VertexType.Z, phase=0, qubit=in1, row=inp_row+2)
    graph.add_vertex_indexed(vetices_full_graph+1)
    graph.set_type(vetices_full_graph+1, VertexType.Z)
    graph.set_phase(vetices_full_graph+1, 0)
    graph.set_row(vetices_full_graph+1, full_graph.row(q_in_1)+1)
    graph.set_qubit(vetices_full_graph+1, in1)
    
    ## Output
    #n_out = graph.add_vertex(VertexType.BOUNDARY, qubit=out, row=full_graph.row(q_in_1)+3)
    graph.add_vertex_indexed(vetices_full_graph+2)
    graph.set_type(vetices_full_graph+2, VertexType.BOUNDARY)
    graph.set_qubit(vetices_full_graph+2, out)
    graph.set_row(vetices_full_graph+2, full_graph.row(q_in_1)+2)
    

    graph.add_edges([(inp1, vetices_full_graph), (vetices_full_graph, vetices_full_graph+1),
                     (inp2, vetices_full_graph+1), (vetices_full_graph+1, vetices_full_graph+2)])

    graph.set_inputs([in1, in2])
    graph.set_outputs([vetices_full_graph+2])
    ...
    return inp1, inp2, vetices_full_graph+2
...
def create_rough_merge(graph: Graph, full_graph: Graph, in1: int, in2: int, out: int, meas_outcome, pauli_frame: PauliFrame=None):
    ...
    graph.set_type(vetices_full_graph, VertexType.Z)
    graph.set_phase(vetices_full_graph, meas_outcome)
    graph.set_row(vetices_full_graph, full_graph.row(q_in_1)+1)
    graph.set_qubit(vetices_full_graph, in1)
    ...
    graph.set_type(vetices_full_graph+1, VertexType.X)
    graph.set_phase(vetices_full_graph+1, 0)
    graph.set_row(vetices_full_graph+1, full_graph.row(q_in_1)+2)
    graph.set_qubit(vetices_full_graph+1, in1)
    ...
    graph.set_type(vetices_full_graph+2, VertexType.BOUNDARY)
    graph.set_qubit(vetices_full_graph+2, out)
    graph.set_row(vetices_full_graph+2, full_graph.row(q_in_1)+3)
...
def compose_by_qubit(g1, g2):
    g2 = g2.copy()

    out_map = {g1.qubit(v): v for v in g1.outputs()}
    in_map = {g2.qubit(v): v for v in g2.inputs()}

    if set(out_map.keys()) != set(in_map.keys()):
        raise ValueError("Mismatched qubit indices between outputs and inputs.")

    plugs = []
    for q in sorted(out_map.keys()):
        o = out_map[q]
        i = in_map[q]

        no = next(iter(g1.neighbors(o)))
        ni = next(iter(g2.neighbors(i)))

        etype =  EdgeType.HADAMARD if (
            g1.edge_type(g1.edge(no, o)) != g2.edge_type(g2.edge(i, ni))
        ) else EdgeType.SIMPLE
        plugs.append((no, ni, etype))

    g1.scalar.mult_with_scalar(g2.scalar)
    
    maxr = max(g1.row(v) for v in g1.vertices() if g1.type(v) != VertexType.BOUNDARY)
    minr = min(g2.row(v) for v in g2.vertices() if g2.type(v) != VertexType.BOUNDARY)
    offset = maxr - minr + 1
    ...
    for v in g2.vertices():
        if v not in in_map.values():
            w = g1.add_vertex(
                g2.type(v),
                phase=g2.phase(v),
                qubit=g2.qubit(v),
                row=g2.row(v) + offset,
                ground=g2.is_ground(v)
            )
            g1.set_vdata_dict(w, g2.vdata_dict(v))
            vtab[v] = w

    for e in g2.edges():
        s, t = g2.edge_st(e)
        if s not in in_map.values() and t not in in_map.values():
            g1.add_edge((vtab[s], vtab[t]), edgetype=g2.edge_type(e))

    for no, ni, et in plugs:
        g1.add_edge((no, vtab[ni]), edgetype=et)

    new_outputs = tuple(vtab[g2.outputs()[i]] for i in range(len(g2.outputs())))
    g1.set_outputs(new_outputs)
